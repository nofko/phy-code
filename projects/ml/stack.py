...
class myNet():
    ...
    def backprop (self , x, y):
        ...
        nabla_b [-1] = delta
        nabla_w [-1] = np.dot(np.array([delta]).transpose() , np.array([activations [ -2]]))
        ...
        for l in range (2, self.N ):
            ...
            nabla_b [-l] = delta
            nabla_w [-l] = np.dot(np.array([delta]).transpose() , np.array([activations [-l -1]]))
...
net.gradient_descent(training_set,30,10,3,test_set)
